...
serve(async (req) => {
  ...
  try {
    ...
    const isScaffalatura = tipoModuloRaw === 'scaffalatura' || tipoModuloRaw.includes('scaffal') || tipoModuloRaw.includes('cantilever')
    if (!isScaffalatura) {
      throw new Error('Questa funzione è dedicata solo alla verifica Scaffalatura')
    }
    ...
    if (record.risposte && typeof record.risposte === 'object') {
      for (const [sezione, domande] of Object.entries(record.risposte)) {
        if (y > 260) { doc.addPage(); y = 20; }
        doc.setFont('helvetica', 'bold')
        doc.setFillColor(255, 193, 7)
        doc.setTextColor(255, 255, 255)
        doc.rect(15, y, 180, 7, 'F')
        doc.text(sezione.toUpperCase().replace(/_/g, ' '), 17, y + 5)
        y += 12
    ...
    const fileName = `verbale_scaffalatura_${record.id}.pdf`
    ...
    const resendApiKey = Deno.env.get('RESEND_API_KEY') || 're_9vyoQUPF_AGCtEg6ALeFDzcyavtiKz4iq'
    ...
    if (record.risposte && typeof record.risposte === 'object') {
      for (const [sezione, domande] of Object.entries(record.risposte)) {
        if (Array.isArray(domande)) {
          riepilogoHtml += `<div style="background-color: #f8f9fa; padding: 12px; border-left: 4px solid #ffc107; margin: 15px 0;"><p style="margin: 0 0 8px 0; font-weight: bold; color: #856404;">${sezione.toUpperCase().replace(/_/g, ' ')}</p>`
          domande.forEach((item: any) => {
            const emoji = item.risposta === 'SI' ? '✅' : '❌'
            riepilogoHtml += `<p style="margin: 4px 0; font-size: 13px;">${emoji} <strong>${item.risposta || 'N.D.'}</strong>: ${item.domanda}</p>`
          })
          riepilogoHtml += '</div>'
        }
      }
...
<div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: auto; border: 1px solid #e0e0e0; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 10px rgba(0,0,0,0.1);">
  <div style="background-color: #856404; padding: 25px; text-align: center;">
    ${logoHtml}
    <h1 style="color: white; margin: 0; font-size: 22px; letter-spacing: 1px;">RAPPORTO SCAFFALATURA</h1>
    <p style="color: #f2e7c1; margin: 5px 0 0 0; font-size: 14px;">Verifica Scaffalatura e Cantilever - Pannelli Termici S.r.l.</p>
  </div>
  <div style="padding: 30px; line-height: 1.6; color: #333; background-color: #ffffff;">
    ...
    <div style="background-color: #f8f9fa; padding: 15px; border-left: 4px solid #ffc107; margin: 20px 0;">
      <p style="margin: 0 0 10px 0; font-weight: bold; color: #856404;">📝 Riepilogo Verifica:</p>
      ${riepilogoHtml}
    </div>
    <div style="text-align: center; margin-top: 35px; padding-top: 25px; border-top: 1px solid #eee;">
      <p style="font-size: 13px; color: #666; margin-bottom: 15px;">Il rapporto completo in formato PDF è disponibile qui sotto:</p>
      <a href="${publicUrl}" style="background-color: #d39e00; color: white; padding: 14px 28px; text-decoration: none; border-radius: 8px; font-weight: bold; display: inline-block; box-shadow: 0 2px 5px rgba(211,158,0,0.3);">📄 SCARICA RAPPORTO PDF</a>
    </div>
  </div>
  <div style="background-color: #f4f4f4; padding: 15px; text-align: center; font-size: 11px; color: #888; border-top: 1px solid #eee;">Questo è un messaggio automatico generato dall'App Verifiche Scaffalatura.<br />Non rispondere a questa email.</div>
</div>`
    ...
    try {
      await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          subject: `RAPPORTO: ${operatore1} - Verifica Scaffalatura e Cantilever`,
          html: emailHtml
        })
      })
    } catch (emailErr) {
