...
serve(async (req) => {
  ...
  try {
    ...
    // =====================
    // HEADER SPECIFICO PRIMO SOCCORSO
    // =====================
    ...
    // =====================
    // CICLO SULLE RISPOSTE (Checklist, Note e Foto)
    // =====================
    ...
    // =====================
    // ESPORTAZIONE PDF E UPLOAD SU STORAGE
    // =====================
    const pdfArray = doc.output('arraybuffer')
    const fileName = `verbale_primo_soccorso_${record.id}.pdf`
    ...
    if (record.risposte && typeof record.risposte === 'object') {
      for (const [sez, domande] of Object.entries(record.risposte)) {
        if (Array.isArray(domande)) {
          riepilogoHtml += `<div style="background-color: #f8f9fa; padding: 12px; border-left: 4px solid #28a745; margin: 15px 0;">
            <p style="margin: 0 0 8px 0; font-weight: bold; color: #28a745;">${sez.toUpperCase().replace(/_/g, ' ')}</p>`;
          domande.forEach((item, i) => {
...
<div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: auto; border: 1px solid #e0e0e0; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 10px rgba(0,0,0,0.1);">
  <div style="background-color: #28a745; padding: 25px; text-align: center;">
    ...
    <p style="color: #e0e0e0; margin: 5px 0 0 0; font-size: 14px;">Verifica Cassette Primo Soccorso - Pannelli Termici S.r.l.</p>
  </div>
  ...
  <div style="padding: 30px; line-height: 1.6; color: #333; background-color: #ffffff;">
    ...
    <div style="background-color: #f8f9fa; padding: 15px; border-left: 4px solid #28a745; margin: 20px 0;">
      <p style="margin: 0 0 10px 0; font-weight: bold; color: #28a745;">📝 Riepilogo Verifica:</p>
      ${riepilogoHtml}
    </div>
  ...
  <div style="background-color: #f4f4f4; padding: 15px; text-align: center; font-size: 11px; color: #888; border-top: 1px solid #eee;">
    Questo è un messaggio automatico generato dall'App Verifiche Primo Soccorso.<br />
    ...
    try {
      await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          subject: `RAPPORTO: ${operatore1} - Verifica Primo Soccorso`,
          html: emailHtml
        })
      })
    } catch (emailErr) {
    ...
    // =====================
    const { error: updateErr } = await supabase.from("verifiche_primo_soccorso")
